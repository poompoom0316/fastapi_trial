...
@app.post("/img_object_detection_to_zip")
def img_object_detection_to_zip(file: bytes = File(...)):
    """
    Object Detection from an image and plot bbox on image. Results are saved as zip-file format.
    ...
    file_prefix = str(dt_now.date())+"-"+str(dt_now.time()).replace(":", "_").replace(".", "_")
    ...
    # here you can choose what data to send to the result
    detect_res = predict[['name', 'confidence', 'xmin', 'ymin', 'xmax','ymax']]
    detect_res.to_csv("bbox.csv", index=False)

    # add bbox on image
    final_image = add_bboxs_on_img(image = input_image, predict = predict)
    final_image.save("bbox_detected.jpg")
    filenames = ["bbox.csv", "bbox_detected.jpg"]
    s = zipfiles(filenames)
